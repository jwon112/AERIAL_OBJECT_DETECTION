...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        ...
        self.nf = ni + nv  # number of files视频加图片数量
        self.video_flag = [False] * ni + [True] * nv#判断是视频还是图片
        self.mode = 'image'
        if any(videos):
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            #path:D:\sourcecode\yolov7\sea_drones_see_detection\images\test\1064.png
            # Read associated JSON metadata only if it exists for the image
            #metadata_path = os.path.splitext(path)[0] + '.json'
            #if os.path.exists(metadata_path):
            #     with open(metadata_path, 'r') as json_file:
            #         self.metadata = json.load(json_file)
            # else:
            #     self.metadata = {}  # Reset metadata to an empty dictionary for non-image files
            #print(f'image {self.count}/{self.nf} {path}: ', end='')

        # Padded resize 填充图像尺寸
        img = letterbox(img0, self.img_size, stride=self.stride)[0]

        # Convert 第二行使得地址连续
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)
        #返回文件路径、处理后的图片、原始图片数据、读取视频文件控制器
        return path, img, img0, self.cap

    def new_video(self, path):
        self.frame = 0#表示当前视频文件第几帧
        self.cap = cv2.VideoCapture(path)#打开视频文件
        self.nframes = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT))#当前视频文件一共多少帧
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        n = len(sources)
        self.imgs = [None] * n
        self.sources = [clean_str(x) for x in sources]  # clean source names for later处理
        ...
        self.rect = np.unique(s, axis=0).shape[0] == 1  # rect inference if all shapes equal 检查视频流尺寸是否相同
        if not self.rect:
