...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        #self.balance = {4: [4.0, 1.0, 0.25, 0.06]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # Regression
                scale = torch.clamp(ps[:, 2:4], min=-1.5, max=1.5)
                pxy = ps[:, :2].sigmoid() * 2. - 0.5
                pwh = (scale.sigmoid() * 2) ** 2 * anchors[i]
...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        lobj *= self.hyp['obj'] #0.7
        lcls *= self.hyp['cls'] #0.2 
        bs = tobj.shape[0]  # batch size
    ...
    def  build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            #pair_wise_iou = BCER(txyxy, pxyxys)
