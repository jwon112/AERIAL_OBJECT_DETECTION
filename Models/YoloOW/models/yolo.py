...
class Detect(nn.Module):
    ...
    def forward(self, x, visualize=False):
        ...
        for i in range(self.nl): 
            ...
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()#(bs, self.na, self.no, ny, nx) -> (bs, self.na, ny, nx, self.no)
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            s = 256  # 2x min stride
            #通过前向传播得到特征图的尺寸，并计算步长s与特征图尺寸的比值，赋值给m.stride。
...
def parse_model(d, ch):  # model_dict, input_channels(3) 
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args 
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, OaohRepConv, TransformerBlock, 
                 SPP, SPPF, SPPCSPC, GhostSPPCSPC, MixConv2d, Focus, Stem, GhostStem, CrossConv, SPPFCSPC,
