...
def parse_args():
    parser = argparse.ArgumentParser(description='Train MSNet')
    ...
    parser.add_argument('--sync_bn', type=str, default='False', help='Sync batch norm')
    parser.add_argument('--fp16', type=str, default='False', help='Use fp16')
    ...
    parser.add_argument('--mixup', type=str, default='True', help='Use mixup augmentation')
    parser.add_argument('--mixup_prob', type=float, default=0.5, help='Mixup probability')
    parser.add_argument('--special_aug_ratio', type=float, default=0.7, help='Special augmentation ratio')
    ...
    parser.add_argument('--Freeze_batch_size', type=int, default=32, help='Freeze batch size')
    parser.add_argument('--UnFreeze_Epoch', type=int, default=400, help='Unfreeze epoch')
    parser.add_argument('--Unfreeze_batch_size', type=int, default=8, help='Unfreeze batch size')
    parser.add_argument('--Freeze_Train', type=str, default='False', help='Freeze training')
    ...
    parser.add_argument('--save_dir', type=str, default='logs/nwpu', help='Save directory')
    parser.add_argument('--eval_flag', type=str, default='True', help='Evaluation flag')
    parser.add_argument('--eval_period', type=int, default=100, help='Evaluation period')
    parser.add_argument('--num_workers', type=int, default=4, help='Number of workers')
    parser.add_argument('--train_annotation_path', type=str, default='annotation_path/NWPU/2007_train.txt', help='Train annotation path')
    parser.add_argument('--val_annotation_path', type=str, default='annotation_path/NWPU/2007_val.txt', help='Validation annotation path')
    ...
    args.cuda = str_to_bool(args.cuda)
    args.distributed = str_to_bool(args.distributed)
    args.sync_bn = str_to_bool(args.sync_bn)
    args.fp16 = str_to_bool(args.fp16)
    args.pretrained = str_to_bool(args.pretrained)
    args.mosaic = str_to_bool(args.mosaic)
    args.mixup = str_to_bool(args.mixup)
    args.Freeze_Train = str_to_bool(args.Freeze_Train)
    args.eval_flag = str_to_bool(args.eval_flag)
...
if __name__ == "__main__":
    ...
    #------------------------------------------------------#
    input_shape     = eval(args.input_shape)  # Convert string to list
    #------------------------------------------------------#
    #   phi             所使用到的yolov8的版本
    #                   n : 对应yolov8_n
    #                   s : 对应yolov8_s
    ...
    #   （二）从0开始训练：
    #       Init_Epoch = 0，UnFreeze_Epoch >= 300，Unfreeze_batch_size >= 16Freeze_Train = False（不冻结训练）
