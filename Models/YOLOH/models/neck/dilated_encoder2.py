...
#수정한 코드
    def forward(self, x):
        x2 = self.projector2_1(x[2]) # x2 : 20x20
        x1 = self.projector1_1(x[1]) # x1 : 40x40
        x0 = self.avgpool_2x2(x[0]) # x0 : 80x80 -> pool 이후 4040
        print(f"[Before Interpolate] x2: {x2.shape}, x1: {x1.shape}, x0: {x0.shape}")


        # [수정] x2 upsample
        if x2.shape[-2:] != x1.shape[-2:]:
            x2 = F.interpolate(x2, size=x1.shape[-2:], mode='bilinear', align_corners=False)
        print(f"[After Interpolate]  x2: {x2.shape}, x1: {x1.shape}, x0: {x0.shape}")

        x = x2 + x1 + x0 #x2 를 x2 업샘플 했기 때문에 모두 40x40 으로 맞춰져서 +가 가능해짐
        x = self.projector2(x)
        x = self.encoders(x)
